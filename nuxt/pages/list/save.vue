...
export default class extends Vue {
  ...
  async getRecommend() {
    ...
    const token = this.$auth.getToken("local");
    const recommend = await CreateListApplication(token).GetRecommend(
      inputedTagName
    );
    if (recommend !== null) {
      this.createRecommend(recommend);
    }
  }

  createRecommend(recommend: Object) {
    Object.keys(recommend).forEach(name => {
      ...
      if (!isRec && !isItm) {
        this.recommends.push({ tagin: recommend[name], name: name });
      }
    });
